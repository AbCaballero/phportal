...
(function ($) {
	Drupal.behaviors.autosave = {
		attach: function(context, settings){
  			...
  			if (!$('form. fieldset legend .fieldset-autosave-status .locked')) {
    			$('form. fieldset legend').append('<span class="fieldset-autosave-status"></span>');
  			}
  			...
  			if (autosaved.serialized) {
    			...
    			$('form. fieldset legend .fieldset-autosave-status').text('Autosave recovered').addClass('autosaved-recovered');
  			}
  			...
  			var allFields = $('form. input[#type!="hidden"][type!="submit"], form. textarea');
  			...
  			//(function ($) {
  				$('form. input[type!="hidden"][type!="submit"], form. select, form. textarea').change(function() {	
    			Drupal.sendAutosave();
  				});
  			//})(jQuery);	
					...
					Drupal.sendAutosave = function() {
      					if (!autosave_lock && autosave_on) {
        					...
        					$.ajax({
            					...
            					autosave_lock = false;
            					$('form .fieldset-autosave-status').text('Changes saved').removeClass('autosaved-recovered').removeClass('not-saved').addClass('changes-saved');
          						}
        					});
      					}	
    				}			
